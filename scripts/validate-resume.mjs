...
if (resumeFiles.length === 0) {
  errors.push("Expected at least one resume markdown file matching src/resume*.md");
}

for (const fileName of resumeFiles) {
  const filePath = path.join(srcDir, fileName);
  const source = fs.readFileSync(filePath, "utf8").replace(/\r\n/g, "\n");
  const fileLabel = `src/${fileName}`;

  const name = source.match(/^#\s+(.+)$/m)?.[1]?.trim();
  const title = source.match(/^##\s+(.+)$/m)?.[1]?.trim();

  if (!name) errors.push(`${fileLabel}: missing top-level name heading (# ...)`);
  if (!title) errors.push(`${fileLabel}: missing title heading (## ...)`);

  for (const section of requiredSections) {
    if (!extractSection(source, section)) {
      errors.push(`${fileLabel}: missing or empty section: ### ${section}`);
    }
  }
  ...
  for (const key of requiredContactKeys) {
    if (!foundContact.has(key)) {
      errors.push(`${fileLabel}: missing contact list item: **${key}:** ...`);
    }
  }
  ...
  if (skills.length < 10) {
    errors.push(`${fileLabel}: expected at least 10 skills in backticks; found ${skills.length}`);
  }
  ...
  for (const [index, row] of skillRows.entries()) {
    if (!/^.+:\s+`[^`]+`/.test(row)) {
      errors.push(`${fileLabel}: skill row #${index + 1} must be 'Category: \`Skill\` ...'`);
    }
  }
  ...
  if (experienceBlocks.length === 0) {
    errors.push(`${fileLabel}: expected at least 1 experience entry starting with '#### '`);
  }

  for (const [index, block] of experienceBlocks.entries()) {
    const lines = block.split("\n").map((line) => line.trim()).filter(Boolean);
    const label = `${fileLabel} experience #${index + 1}`;
  ...
  if (!referencesSection || referencesSection.length < 5) {
    errors.push(`${fileLabel}: references section appears empty`);
  }
}
