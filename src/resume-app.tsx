...
const parseSkillGroups = (section: string): SkillGroup[] =>
  section
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean)
...
const parseResumeMarkdown = (source: string): ResumeData => {
  ...
  const skillGroups = parseSkillGroups(extractSection(normalized, "Skills"));
      ...
      <section className="skills">
        ...
        <ul className="skill-groups">
          {data.skillGroups.map((group) => (
            <li className="skill-group" key={group.label}>
        ...
        <ul>
          {data.interests.map((interest, index) => (
            <li key={interest.title}>
              {interestIcons[index % interestIcons.length]}
              <div>
