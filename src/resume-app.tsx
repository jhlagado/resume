...
const parseSkills = (section: string): SkillCategory[] =>
  section
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean)
    .map((line) => {
      ...
      const skills = [...rest.join(":").matchAll(/`([^`]+)`/g)].map((match) => match[1].trim());
...
const parseResumeMarkdown = (source: string): ResumeData => {
  ...
  const skills = parseSkills(extractSection(normalized, "Skills"));
  const experiences = parseExperiences(extractSection(normalized, "Experience"));
...
export const ResumeApp = ({ markdown, lastUpdated }: ResumeAppProps) => {
        ...
        <ul className="contact">
          <li>
            ...
            {data.contact.Availability && (
              <div>
                <b>Availability:</b> <LinkOrText value={data.contact.Availability} />
              </div>
            )}
            <div>
              <MdLocationOn />
      ...
      <section className="skills">
        ...
        <div className="skill-categories">
          {data.skills.map((category) => (
            <section className="skill-category" key={category.label}>
